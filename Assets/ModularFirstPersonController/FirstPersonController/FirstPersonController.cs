...
public class FirstPersonController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            if (Mobile)
            {
                targetVelocity = new Vector3(MovementJoystick.Horizontal, 0, MovementJoystick.Vertical);
            }
            else
            {
            ...
            if (enableSprint && Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown || enableSprint && isSprinted && sprintRemaining > 0f && !isSprintCooldown)
            {
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpc.MovementJoystick = (Joystick)EditorGUILayout.ObjectField(new GUIContent("Movement Joystick", "Movement Joystick."), fpc.MovementJoystick, typeof(Joystick), true);
        fpc.playerCanMove = EditorGUILayout.ToggleLeft(new GUIContent("Enable Player Movement", "Determines if the player is allowed to move."), fpc.playerCanMove);
        fpc.Mobile = EditorGUILayout.ToggleLeft(new GUIContent("Is Mobile", "is mobile"), fpc.Mobile);
